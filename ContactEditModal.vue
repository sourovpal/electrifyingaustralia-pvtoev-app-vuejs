<script>
    ...
    import { icons } from './src/asset/svgicon';
    import {
        CreateLeadContact,
        UpdateLeadContact,
        SearchLeadContact,
    } from './src/actions/LeadAction'
                            ...
                            <ul class="contacts-list list-unstyled">
                                <li v-for="(item, index) in contacts"
                                    ...
                                    <div class="contact-details">
                                        <span class="details-text text-head fs-16px d-block fw-bold">{{ item.full_name
                                            }}</span>
                                        <span class="details-text text-soft fs-14px d-block">{{
                                            item.email??item.phone_number??'example@gmail.com' }}</span>
                                    </div>
                                </li>
                            </ul>
                            <button v-if="contact"
                                @click="selectContactHandler()"
                                class="btn btn-primary w-100">Add New</button>
                        </div>
                    </div>
                    <div class="col-lg-8">
                        <div class="modal-header py-2 px-0">
                        ...
                        <div class="modal-body px-0">
                            ...
                            <div class="row">
                                ...
                                <div class="col-lg-4 mb-3 mb-lg-0">
                                    <div class="">
                                        ...
                                        <select v-model="phone_use"
                                            class="form-control">
                                            <option value=""></option>
                                            <option v-for="(item, index) in intendedUse"
                                                :key="index"
                                                :selected="phone_use==item"
                                    ...
                                    <div class="">
                                        ...
                                        <select @change="item.phone_use=$event.target.value"
                                            ...
                                            <option v-for="(intended, index) in intendedUse"
                                                ...
                                                :value="intended">{{ intended }}</option>
                                        </select>
                                    </div>
                                </div>
                            </div>
                            <div class="row mb-1">
                                <div class="col-lg-8"><span class="fs-14px text-danger py-1 w-100 d-block"
                                        v-if="errors?.phone_number?.length">{{ errors?.phone_number[0] }}</span></div>
                                <div class="col-lg-4">
                                    <div @click="addedAnatherPhoneHandler()"
                                        class="fs-14px text-primary cursor-pointer py-1 select-none">Add another phone
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="row">
                                ...
                                <div class="col-lg-4 mb-3 mb-lg-0">
                                    <div class="">
                                        <label class="form-label-title">Intended use</label>
                                        <select v-model="email_use"
                                            class="form-control">
                                            <option value=""></option>
                                            <option v-for="(item, index) in intendedUse"
                                                :key="index"
                                                :selected="email_use==item"
                                                :value="item">{{ item }}</option>
                                        </select>
                                    </div>
                                </div>
                            ...
                            <div class="row mb-1">
                                <div class="col-lg-8"><span class="fs-14px text-danger py-1 w-100 d-block"
                                        v-if="errors?.email?.length">{{ errors?.email[0] }}</span></div>
                                <div class="col-lg-4">
                                    <div @click="addedAnatherEmailHandler()"
                                        class="fs-14px text-primary cursor-pointer py-1 select-none">Add another email
                                    </div>
                                </div>
                            </div>
                            <div class="mb-3">
                                <label class="form-label-title">Notes <span
                                        class="text-soft fs-12px ms-1">(Optional)</span></label>
                                    ...
                                    <ul class="list-unstyled mb-0 search-contacts">
                                        <li @click="createLeadContactHandler(item)"
                                            ...
                                            <span class="d-block fs-14px text-head fw-bold">
                                                {{ item.title }} <span v-if="item.title && item.full_name"> - </span> {{
                                                item.full_name }}
                                            </span>
                                            <span class="d-block fs-12px text-soft">
                                                {{ item.phone_number }} <span v-if="item.email && item.phone_number"> -
                                                </span> {{ item.email }}
                                            </span>
                                        </li>
                                    </ul>
