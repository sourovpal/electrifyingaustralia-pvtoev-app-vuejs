...
<template>
  <div class="alert-body" v-if="isLoading && !notifications?.length">
    <notification-skeletor
      v-for="(_, index) in 6"
      :key="Math.random()"
    ></notification-skeletor>
  </div>
  ...
  >
    <div v-if="!hideAll" class="alert-body">
      <notification-item
        v-for="(notification, index) in notifications"
        :key="index"
        :notification="notification"
      ></notification-item>
      <div ref="infiniteLoadRef" class="loading-notification">
        <notification-skeletor
          v-if="!isComplete"
          v-for="(_, index) in 2"
          :key="Math.random()"
      ...
      <div v-if="!isLoading" class="py-5"></div>
    </div>
  </custom-scrollbar>
</template>
<style scoped
