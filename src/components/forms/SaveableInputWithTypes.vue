...
const props = defineProps<{
    ...
    amountFormatter?: (num: number) => string|number,
    inputType?: 'number' | 'text' | 'select'
...
const inputValue = ref<null|string|number>(null);
const inputValid = computed<boolean>(() => {
    if (!inputValue.value) 
        return false;

    const stringified: string = inputValue.value.toString();
...
const selectList = ref<OptionListItem[]>(
...
const selectedOption = computed<string|null>(() => {
    const selectedValue: OptionListItem | undefined = selectList.value?.find(
        (item: OptionListItem) => item.id === props.modelValue
    )
...
const handleSelectListOptionClick = (optionListItem: OptionListItem) => inputValue.value = optionListItem.id;
