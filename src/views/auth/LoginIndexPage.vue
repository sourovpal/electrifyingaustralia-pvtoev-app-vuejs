<script setup>
  ...
  async function submitLoginForm(e) {
    ...
    try {
      ...
      const { success, user, access_token, message } = res;
      if(success){
...
<template>
  <div class="body-img"
    style="background-image: url(https://images.pexels.com/photos/2800845/pexels-photo-2800845.jpeg);">
            ...
            <div class="login-wrap p-0">
              <form @submit.prevent="submitLoginForm($event)"
                ...
                <div class="form-group mb-3">
                  <input @focus="delete data.errors?.username"
                    v-model="form.username"
                    type="text"
                    class="login-form-control"
                    placeholder="Username or Email Address">
                  <span class="text-center fs-14px text-danger py-1 w-100 d-block"
                    v-if="data.errors?.username?.length">{{ data.errors?.username[0] }}</span>
                </div>
                <div class="form-group mb-3">
                  <input @focus="delete data.errors?.password"
                  ...
                  <span class="text-center fs-14px text-danger py-1 w-100 d-block"
                    v-if="data.errors?.password?.length">{{ data.errors?.password[0] }}</span>
                </div>
                ...
                <div class="form-group">
                  <div class="w-100 text-left">
                    <span>Don't have an account ? <router-link to="/register"
                        style="color: #1662ee">Sign Up.</router-link></span>
                  </div>
                </div>
              </form>
            </div>
