...
const handleSearchContacts = useDebounceFn((search) => {
  ...
  if (
    contactPagination.value?.current_page &&
    contactPagination.value?.current_page > 1
  ) {
...
function selectEditContact(contact) {
  ...
  if (contact.lead) {
    ...
    toggleContactEditModal.value = true;
    contactEditModal.value = contact;
  }
}
  ...
  <section class="content contacts-list">
    ...
    <Datatable v-else>
      <datatable-header>
        ...
        <div class="tbl-th" style="width: 15rem; flex-grow: 1">
          Email Address
        </div>
      ...
      <datatable-body>
        ...
        >
          ...
          <div style="width: 10rem; flex-grow: 1" class="tbl-td">
            {{ handleDateTimeFormat(item?.updated_at) }}
          </div>

          <div style="width: 10rem; flex-grow: 1" class="tbl-td">
            {{ handleDateTimeFormat(item?.created_at) }}
          </div>
        </div>
      </datatable-body>
    </Datatable>
