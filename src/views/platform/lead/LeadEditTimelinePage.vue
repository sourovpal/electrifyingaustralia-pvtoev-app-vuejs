      ...
      <right-action-bar>
        ...
        <div class="settings-group-item owner-list-dropdown me-3 d-none d-lg-inline">
          <button class="owner-dropdown-toggler"
            ...
            aria-expanded="false">
            <div class="icon">
              <img :src="owner?.profile_avatar??AvatarIcon"
                alt="" />
            </div>
          </button>
          <DropdownSubscriberList :owner="owner"
            :owners="owners" />
        </div>
            ...
            <div class="btn-group shadow-0 white-space-nowrap w-100 overflow-auto">
              <button v-for="(stage, index) in leadStages"
                ...
                :class="{
                  complete:
                    ((stage?.lead_stage &&
                    currentLead?.pipeline_stage_id != stage?.stage_id) || stage.position < currentLead?.pipeline_stage?.position ),
                  active: currentLead?.pipeline_stage_id == stage?.stage_id
                }">
