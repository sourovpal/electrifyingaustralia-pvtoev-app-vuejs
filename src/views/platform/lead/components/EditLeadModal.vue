    ...
    <div class="modal-dialog modal-dialog-centered modal-lg">
        <div class="modal-content mb-5">
            ...
            <div class="modal-body pt-lg-0">
                <ul class="nav nav-tabs nav-justified mb-3 d-none d-lg-flex" id="ex1" role="tablist">
                    <li class="nav-item nav-link text-capitalize cursor-pointer fs-14px fw-bold text-hard" :class="toggleTabs==1?'active':''"  @click="toggleTabs=1">Show All</li>
                    <li class="nav-item nav-link text-capitalize cursor-pointer fs-14px fw-bold text-hard" :class="toggleTabs==2?'active':''" @click="toggleTabs=2">Lead Properties</li>
                    <li class="nav-item nav-link text-capitalize cursor-pointer fs-14px fw-bold text-hard" :class="toggleTabs==3?'active':''" @click="toggleTabs=3">Custom Properties</li>
                </ul>
                    ...
                    <div class="row">
                        <div class="col-lg-9">
                            <div class="mb-3 position-relative">
                                ...
                                class="form-control" type="text" data-mdb-toggle="dropdown">
                                <div class="dropdown-menu fade custom-form-select overflow-auto" style="max-height:125px;">
                ...
                <div v-show="toggleTabs != 2">
                    <div class="py-3" v-if="toggleTabs==1">
                        <h6 class="fs-16px fw-bold text-soft">Custom properties</h6>
                    </div>

                    <div class="mb-3 position-relative" v-for="(propertie, index) in leadProperties" :key="index">

                        <label class="form-label-title" for="">{{ propertie.label }}</label>

                        <!-- type text and input tag -->
                        <input v-if="propertie?.data_type_id == 'free_text' || propertie?.data_type_id == 'read_only'" 
                        ...
                        <textarea 
                        v-if="propertie?.data_type_id == 'multiline_free_text'" 
                        @input="formDataHandler($event, propertie?.unique_id, true)" 
                        ...
                        <input 
                        v-if="propertie?.data_type_id == 'date'" 
                        @input="formDataHandler($event, propertie?.unique_id, true)" 
                        ...
                        <!-- type data and input tag -->
                        <input 
                        v-if="propertie?.data_type_id == 'date_and_time'" 
                        @input="formDataHandler($event, propertie?.unique_id, true)" 
                        class="form-control" type="datetime-local" 
                        :value="leadCustomProperties[propertie?.unique_id]">
                        
                        <!-- type checkbox and input tag -->
                        <div v-if="propertie?.data_type_id == 'yes_or_no'" class="btn-group">
                            <button 
                            @click="yesOrNoHandler('1', propertie?.unique_id)" 
                            class="btn btn-sm"
                            :class="`${(formData[propertie?.unique_id]??leadCustomProperties[propertie?.unique_id]) == '1'?'btn-primary':'btn-warning'}`"
                            >Yes</button>
                            <button 
                            @click="yesOrNoHandler('0', propertie?.unique_id)" 
                            class="btn btn-sm"
                            :class="`${(formData[propertie?.unique_id]??leadCustomProperties[propertie?.unique_id]) == '0'?'btn-primary':'btn-warning'}`"
                            >No</button>
                        </div>
                        ...
                        class="form-control cursor-pointer" type="text" data-mdb-toggle="dropdown" readonly 
                        ...
                        <div class="dropdown-menu fade custom-form-select overflow-auto" style="max-height:125px;">
                            <ul class="list-unstyled mb-0">
                                ...
                                @click="formData[propertie?.unique_id] && singleChoiceHandler(null, propertie?.unique_id)"><i>Select {{ propertie.label }}</i></li>
                                <li 
                                v-for="(item, index) in  propertie?.attributes?.values??[]" 
                                :key="index"
                        ...
                        <div class="dropdown-menu fade custom-form-select overflow-auto" style="max-height:125px;">
                            <ul class="list-unstyled mb-0">
                                <li class="dropdown-item text-hard fw-bold fs-14px py-1" 
                                @click="formData[propertie?.unique_id] && multipleChoiceHandler(null, propertie?.unique_id)"><i>Select {{ propertie.label }}</i></li>
                                <li 
                                v-for="(item, index) in  propertie?.attributes?.values??[]" 
                                :key="index"
