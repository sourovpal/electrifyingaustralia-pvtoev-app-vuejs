<template>
  <div class="custom-select dropdown position-relative">
    ...
    <div v-if="!selectedOptions?.length && usebg && !isOpen && !isEdit"
      @click="isEdit=!isEdit"
      class="form-control form-control-sm form-control-tags-input"
      :class="{bg:usebg}">
      <span class="fw-bold text-head d-flex justify-content-center align-items-center">–</span>
    </div>

    <div v-else
      class="form-control form-control-sm form-control-tags-input px-1"
      :class="{active:isOpen, bg:usebg}"
      @click="toggleDropdown">
      <label v-for="(option, index) in selectedOptions"
        :key="index"
        class="tag-lable text-head">
        {{ option }} <span @click.stop="removeOption(option)"
          class="close-tag">&times;</span>
      </label>
    </div>
