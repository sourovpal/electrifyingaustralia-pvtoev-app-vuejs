...
<div class="modal fade add-new-lead-modal" id="allFilesShowRef" ref="allFilesShowRef" aria-hidden="true" aria-labelledby="allFilesShowRef" tabindex="-1">
    <div class="modal-dialog modal-dialog-centered modal-xl">
        <div class="modal-content">
            <div class="modal-header py-2">
                <div class="d-flex justify-content-center align-items-center py-0">
                    <ul class="m-0 list-unstyled p-0 d-flex justify-content-start align-items-center tabs-list">
                        <li @click="selectFileTypeHandler('all')" :class="{active:(activeTab == 'all')}" class="px-1 me-1 cursor-pointer fs-14px text-head">All</li>
                        <li @click="selectFileTypeHandler('images')" :class="{active:(activeTab == 'images')}" class="px-1 me-1 cursor-pointer fs-14px text-head">Images</li>
                        <li @click="selectFileTypeHandler('pdf')" :class="{active:(activeTab == 'pdf')}" class="px-1 me-1 cursor-pointer fs-14px text-head">Pdf</li>
                        <li @click="selectFileTypeHandler('others')" :class="{active:(activeTab == 'others')}" class="px-1 me-1 cursor-pointer fs-14px text-head">Others</li>
                    </ul>
                </div>
                <div>
                    <button class="btn btn-light btn-sm btn-floating" @click="hideModalHandler()">
            ...
            <div class="modal-body">
                <div class="file-box">
                    <div class="file-item" v-for="(item, index) in showFiles" :key="index">
                        <div class="file" @click="showPreviewModalHandler(item)">
                            <img v-if="fetchFileHandler(item)" v-lazy="item?.file_path">
                            <span :class="{pdf:(item?.extension == 'pdf'), others:(item?.extension != 'pdf')}" v-else>{{ item?.extension?.toUpperCase() }}</span>
                        </div>
