...
<template>
    <div>
            ...
            <div class="dropdown-header py-2 px-3 d-flex justify-content-between align-items-center" :class="{show:toggleDropdownBox}">
                ...
                <div class="">
                    ...
                    @click="$refs['ShowAllFilesModalRef'].showModalHandler()">
                        <svg class="svg-5" xmlns="http://www.w3.org/2000/svg" height="18px" viewBox="0 0 24 24" width="18px" fill="#000000"><path d="M0 0h24v24H0z" fill="none"></path> <path d="M19 19H5V5h7V3H5c-1.11 0-2 .9-2 2v14c0 1.1.89 2 2 2h14c1.1 0 2-.9 2-2v-7h-2v7zM14 3v2h3.59l-9.83 9.83 1.41 1.41L19 6.41V10h2V3h-7z"></path></svg>
                    </button>
                            ...
                            <div class="">
                                ...
                                <div class="files-progress-list">
                                    <div class="item mb-2" v-for="(file, index) in files" :key="index" v-show="file?.status != 2">
                                        ...
                                        <div class="progress">
                                            <div class="progress-bar" :class="`${file?.status == 0 ?'bg-danger':'bg-success'}`" :style="`width:${file?.progress}%;border-radius: 5px;`"></div>
                                        </div>
                                    </div>
                                </div>
                                <ul class="list-unstyled file-list">
                                    <li 
                                    class="file-list-item cursor-pointer pt-1 border-bottom pb-1 d-flex justify-content-between align-items-center text-head" 
                                    v-for="(item, index) in leadFiles?.slice(0, 10)" :key="index">
                                    ...
                                    <li v-if="leadFiles?.length > 5" class="text-center py-2"><a @click="$refs['ShowAllFilesModalRef'].showModalHandler()" class="fs-14px cursor-pointer text-primary">Show All Files</a></li>
                                </ul>
                            </div>
                        </div>
                    </div>
        ...
        <div class="file-upload-history" v-if="toggleUploadHistoryDropdown != 0" :class="{show:(toggleUploadHistoryDropdown==2)}">
            <div class="history-header" @click="toggleUploadHistoryHandler(toggleUploadHistoryDropdown)">
                <div class="title text-white fs-14px">{{ upload_title }}</div>
                <div class="toggler">
                    <button @click="toggleUploadHistoryHandler(0)" class="btn btn-sm btn-light btn-md btn-lg btn-floating bg-transparent toggle-btn">
                        <svg fill="currentColor" xmlns="http://www.w3.org/2000/svg" height="14" viewBox="0 -960 960 960" width="14"><path d="m256-200-56-56 224-224-224-224 56-56 224 224 224-224 56 56-224 224 224 224-56 56-224-224-224 224Z"></path></svg>
                    </button>
                </div>
            </div>
                    ...
                    <div class="item mb-2" v-for="(file, index) in files" :key="index">
                        <div class="d-flex justify-content-between align-items-center">
                            <div class="fs-14px text-head">
                                <img class="me-2" width="24" :src="getFileIconHandler(file?.extension)" alt="">
                                {{shortenFileNameHandler(file?.name, 12) }}
                            </div>
                            <div class="fs-14px text-soft" v-if="file.status != 2">{{ file?.progress }}%</div>
                            <div class="fs-14px text-soft" v-else-if="file.status == 2">
                                <svg xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 0 24 24" width="24px" fill="#239B56"><path d="M0 0h24v24H0z" fill="none"></path> <path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm-2 15l-5-5 1.41-1.41L10 14.17l7.59-7.59L19 8l-9 9z"></path></svg>
                            </div>
                        </div>
