...
export default {
  components: {
    ...
    ShowAllFilesModal,
    SlideUpDown,
    FilesUploadedHistory,
  },
  mixins: [UploadedMixin],
  setup() {
  ...
  data() {
    return {
      ...
      dragStart: false,
      uploadedHistoryTitle: "Uploaded Files",
    };
  },
  computed: {
    leadFiles: {
      get() {
        return this.leadStore.getLeadFiles;
      },
      set(files) {
  ...
  methods: {
    ...
    browseFilesHandler() {
      try {
        ...
        input.setAttribute(
          "accept",
          ".docx, .xlsx, .pdf, .xls, .csv, .gif, .txt, .png, .jpg, .jpeg, .webp, .bmp, .tiff, .svg"
        );
        input.click();
      ...
      try {
        ...
        try {
          ...
          try {
            ...
            this.deletedFiles = deleted_lead_files;
    ...
    getFileUploadTitle() {
      ...
      if (uploadLength > 0) {
        ...
        title = `${completeLength} file${
          completeLength > 1 ? "s" : ""
        } upload complete`;
      }
      this.uploadedHistoryTitle = title;
    },
...
<template>
  <div>
    <div class="dropdown-box border-bottom">
      ...
      >
        ...
        <div class="">
          ...
          <button
            class="btn btn-sm btn-light btn-md btn-lg btn-floating bg-transparent toggler"
            ...
            <div class="lead-files">
              ...
              >
                ...
                <div class="empty-state__body">
                  ...
                  <button @click="browseFilesHandler()" class="btn-text">
                    Browse files
                  </button>
                </div>
              </div>
              <div class="">
                  ...
                  >
                    ...
                    >
                      <div class="fs-14px text-head">
                        {{ resizeFileNameHandler(file?.name) }}
                      </div>
                    ...
                    <div class="progress">
                      <div
                        class="progress-bar"
                        :class="`${
                          file?.status == 0 ? 'bg-danger' : 'bg-success'
                        }`"
                ...
                <ul class="list-unstyled file-list">
                  ...
                  >
                    <div
                      @click="
                        $refs['imagePreviewModalRef'].showModalHandler(item)
                      "
                    >
                      <img
                        class="me-2"
                        width="24"
                        :src="getFileIconByFileExtension(item?.extension)"
                        alt=""
                      />
                      <span class="fs-12px">{{
                        resizeFileNameHandler(item?.name)
                      }}</span>
                    </div>
                  ...
                  <li v-if="leadFiles?.length > 5" class="text-center py-2">
                    <a
                      @click="$refs['ShowAllFilesModalRef'].showModalHandler()"
                      class="fs-14px cursor-pointer text-primary"
                      >Show All Files</a
                    >
                  </li>
                </ul>
    ...
    <files-uploaded-history :title="uploadedHistoryTitle" :files="files" />

    <ImagePreviewModal :files="leadFiles" ref="imagePreviewModalRef" />

    <show-all-files-modal
      :files="leadFiles"
      :imagePreview="
        (item) => $refs['imagePreviewModalRef'].showModalHandler(item)
      "
