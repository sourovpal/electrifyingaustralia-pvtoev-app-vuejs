<script setup>
    ...
    async function handleLeadsDelete() {
        ...
        if (!inputTotalDeleteLeads.value) {
            return errors.value = { 'lead_number': [`Please confirm by typing the number <strong>( ${selectedLeads.value?.length} )</strong> in the box below .`] };
        } else if (inputTotalDeleteLeads.value != selectedLeads.value?.length) {
            return errors.value = { 'lead_number': ['The number you entered does not match the total selected leads.'] };
        }
            ...
            <div class="modal-content"
                ...
                <div class="modal-header py-2">
                    <div class="d-flex justify-content-center align-items-center py-0">
                        <font-awesome-icon icon="fas fa-trash"
                            class="fs-16px text-head me-2"></font-awesome-icon>
                ...
                <div class="modal-body">
                    <p class="text-center fs-14px">
                        ...
                        To confirm your intent, please type the number of leads <strong>
                    ...
                    <div>
                        ...
                        <span class="fs-14px text-danger py-1 w-100 d-block"
                            ...
                            v-if="errors?.lead_number?.length"></span>
                    </div>
                </div>
                <div class="modal-footer d-flex justify-content-between align-items-center border-0">
                    ...
                    <div>
                        <loading-button class="btn-sm btn-warning"
                            :is-loading="isLoading"
                            @submit="handleLeadsDelete">Confirm?</loading-button>
                    </div>
                </div>
            </div>
        </div>
