    ...
    export default {
        ...
        data() {
            return {
                ...
                orderByAscOrDesc: false, // ASC True
                pipeline: {},
                stages: [],
                owners: [],
                leadSources: [],
                ...
                currentSortedOrder:{},
                ascending: [
                    { field: 'updated_at', order: 'asc', title: 'Least recently updated first' },
                    { field: 'created_at', order: 'asc', title: 'Least recently created first' },
                    { field: 'estimated_value', order: 'asc', title: 'Least valuable first' },
                    { field: 'confidence', order: 'asc', title: 'Least stars first' },
                ],
                descending: [
                    { field: 'updated_at', order: 'desc', title: 'Most recently updated first' },
                    { field: 'created_at', order: 'desc', title: 'Most recently created first' },
                    { field: 'estimated_value', order: 'desc', title: 'Most valuable first' },
                    { field: 'confidence', order: 'desc', title: 'Most stars first' },
                ],
        ...
        methods: {
            currentSortedOrderHandler(item) {
                this.currentSortedOrder = item;
                this.fetchPipelineWithStages({ field: item?.field, order: item?.order });
            },
            ...
            resetFilterSidebar(show) {
                try {
                    ...
                    if (show) {
                        ...
                        this.$refs['toggleFilterSidebarRef'].resetFilterHandler();
                        this.toggleFilterSidebar = false;
                        if (this.countTotalFilters) {
            ...
            async fetchPipelineWithStages(payload = {}) {
                try {
                    ...
                    if (this.countTotalFilters) {
                        this.payload['search'] = JSON.stringify(this.filterQueryData);
                    }
        ...
        mounted() {
            this.isMounted = true;
            this.currentSortedOrder = this.descending[0];
            this.fetchPipelineWithStages();
        ...
        computed: {
            countTotalFilters() {
                return Object.keys(this.filterQueryData).length;
            }
        }
    }
</script>
<template>
    <section class="content pipline-list">
                    ...
                    <div class="d-flex flex-row justify-content-start align-items-center">
                        ...
                        <div class="mx-2">
                            <router-link :to="{query:{...$route.query,view:'row' }}">
                                <button type="button"
                                    class="toolbar-btn btn btn-light btn-lg btn-floating">
                                    <svg :class="` ${!pipelineViewColumns?'active-svg-tbl':''} svg-5`"
                                        xmlns="http://www.w3.org/2000/svg"
                                        height="24"
                                        viewBox="0 0 24 24"
                                        width="24">
                ...
                <div class="col-md-6">
                    <div class="d-flex flex-row justify-content-end align-items-center">
                        ...
                        <div class="btn-group me-3"
                            v-if="countTotalFilters">
                            <button class="btn btn-success btn-sm">
                                {{ countTotalFilters }} active filter
                            </button>
                            <button @click="resetFilterSidebar(false)"
                                class="btn btn-success btn-sm px-2 active">
                                <svg v-if="toggleFilterSidebar"
                                    xmlns="http://www.w3.org/2000/svg"
                                    height="18px"
                                    viewBox="0 0 24 24"
                                    width="18px"
                        ...
                        <div class="dropdown-sorted">
                            <button data-mdb-toggle="dropdown"
                                ...
                                {{ currentSortedOrder?.title }}
                            </button>
                            <div class="dropdown-menu dropdown-menu-end dropdown-sorted-list"
                                style="width:18rem;"
                                @click="(event)=>event.stopPropagation()">
                                <div class="d-flex justify-content-between align-items-center p-2">
                                    <button @click="orderByAscOrDesc = false"
                                        class="btn btn-sm"
                                        :class="!orderByAscOrDesc?'btn-primary':'btn-outline-primary'">
                                        <span class="me-1">
                                            <svg xmlns="http://www.w3.org/2000/svg"
                                                height="18px"
                                                viewBox="0 0 24 24"
                                    ...
                                    <button @click="orderByAscOrDesc = true"
                                        class="btn btn-sm"
                                        :class="orderByAscOrDesc?'btn-primary':'btn-outline-primary'">
                                        <span class="me-1">
                                            <svg xmlns="http://www.w3.org/2000/svg"
                                ...
                                <div class="tab-content"
                                    id="ex1-content">
                                    <ul :class="{'fade show active':orderByAscOrDesc}"
                                        class="list-unstyled mb-0 ascending-menu tab-pane">
                                        <li v-for="(item, index) in ascending"
                                            :key="index"
                                            :class="{active:(currentSortedOrder?.field == item['field'] && currentSortedOrder?.order == item['order'])}"
                                            @click="currentSortedOrderHandler(item)">
                                    ...
                                    <ul :class="{'fade show active':!orderByAscOrDesc}"
                                        ...
                                        <li v-for="(item, index) in descending"
                                            :key="index"
                                            :class="{active:(currentSortedOrder?.field == item['field'] && currentSortedOrder?.order == item['order'])}"
                                            @click="currentSortedOrderHandler(item)">
                                            <a class="dropdown-item cursor-pointer">{{ item?.title }}</a>
                        ...
                        <div class="ms-2">
                            <button @click="toggleFilterSidebar = !toggleFilterSidebar"
                                type="button"
                                class="toolbar-btn btn btn-light btn-lg btn-floating">
                                <svg class="svg-5"
                                    xmlns="http://www.w3.org/2000/svg"
        ...
        <FilterRightSidebar :class="{show:toggleFilterSidebar}"
            ref="toggleFilterSidebarRef"
            :owners="owners"
            :leadSources="leadSources"
            :toggle-filter="(e)=> toggleFilterSidebar = e"
            @filter-data-in-database="(filter)=> filterDataInDatabase(filter)" />
        <div v-if="pipelineViewColumns"
            class="pip-body-scrollbar"
                ...
                <div class="piplien-body d-flex flex-row">
                    ...
                    <div class="piplien-state"
                        ...
                        <div class="pip-header ps-3 pe-2 py-2 d-flex flex-column">
                            <h3 class="fs-18px text-head fw-bold mb-0 fw-bold text-overflow-ellipsis">{{ stage.name }}
                            </h3>
                        ...
                        <CustomScrollbar :simulateScroll="true"
                            thumbWidth="3.5"
                            direction="horizontal"
                            @scroll="infiniteLoadedLeads($event, stage.id)">
                            <div class="pip-body px-2">
                                <component v-for="(component, index) in components[stage.id]??[]"
                                    :key="index"
                                    :is="`pipeline-state-lead-details`"
                                    :data="component.data"
                                    :index="index" />
                                <loading-state-leads v-if="leadLastId[stage.id]"
                                    :col-size="2" />
                            </div>
                        </CustomScrollbar>
                    </div>
                    ...
                    <div class="piplien-state"
                        ...
                        <div class="pip-header ps-3 pe-2 py-2 d-flex flex-column">
                            <h3 class="fs-18px text-head fw-bold mb-0"
                                style="opacity: 0;">-</h3>
                            <span class="fs-12px text-head"
                                style="opacity: 0;">-</span>
                        </div>
                        <div class="py-1 value-bar">
                            <div class="px-3 d-flex justify-content-between align-items-center">
                                <span class="text-head fw-bold fs-12px"
                                    style="opacity: 0;">-</span>
                                <span class="text-head fw-bold fs-12px"
                                    style="opacity: 0;">-</span>
                            </div>
