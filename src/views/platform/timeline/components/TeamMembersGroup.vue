<script setup>
    ...
    const onSubmitTeamMembers = async (member) => {
        ...
        is_loading.value = member.user_id;
    ...
    const filterUsers = computed(() => {
        ...
        return users.value?.filter((user) => {
            ...
            if (user.name?.toLowerCase()?.search(search.value?.toLowerCase()) > -1 ||
...
<template>
    <avatar-group>
        ...
        <Avatar v-for="(member, index) in groupMembers"
            :key="index"
            :image="member.profile_avatar"
            v-tippy="{ content: member.name || member.email, placement: 'top' }"
            size="small"
            shape="circle"
            class="me-1"
        ...
        <div class="">
            ...
            <div>
                ...
                <ul class="list-unstyled m-0 "
                    ...
                    <li v-for="member in filterUsers"
                        :key="member.name"
                        class="d-flex flex-row justify-context-start align-items-center px-3 py-1 dropdown-item">

                        <div class="me-2">

                            <custom-checkbox @click="onSubmitTeamMembers(member)"
                                :checked="isTeamMember(member)"
                                class="btn-sm">

                                <circle-spinner v-if="is_loading == member.user_id"
                                    :loading="is_loading == member.user_id" />
