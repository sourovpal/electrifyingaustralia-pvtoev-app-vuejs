...
async function selectPipelineHandler(pipelineOption) {
  ...
  await useApiRequest({
    url: `/stages/${pipelineOption.pipeline_id}/${
      props.isLost ? "lost" : "success"
    }`.trim(),
  })
    .then((res) => {
...
async function handleSubmitLostOrWon() {
  ...
  let url = `/platform/${editLeadId.value}/lost-won`;
  let payload = {
    ...
    <template #header>
      <div class="modal-header py-1">
        <h6 class="mb-0 fw-bold">
          {{ isLost ? "Lost deal's" : "Won deal's" }}
        </h6>
        <button
          data-mdb-dismiss="modal"
          class="btn btn-light btn-sm btn-floating d-flex justify-content-center align-items-center"
        >
          <font-awesome-icon icon="fas fa-close" class="fs-14px text-soft" />
        </button>
      </div>
    </template>
    <template #body>
      <div v-if="!isLost" class="mb-3">
        <label class="mb-2 fs-16px text-head">Pipeline</label>
        <select-object
      ...
      <div class="mb-3">
        <label class="mb-2 fs-16px text-head">
          {{ `${props.isLost ? "Lost " : "Success "}` }} Stage</label
        >
        <select-object
      ...
      <div v-if="!isLost" class="mb-3 position-relative">
        <label class="mb-2 fs-16px text-head"> Change Owner</label>
        <div
          class="form-control cursor-pointer owner-form-control select-none"
      ...
      <div class="mb-3">
        ...
        <textarea v-model="commant" class="form-control" rows="4"></textarea>
      </div>
