...
<template>
      ...
      <div class="dropdown-body">
        <div class="px-3 py-1 pb-2">
          ...
          <Task
            ...
            :task="{}"
            :key="Math.random()"
            @toggle-new-task="(_status) => (isCreateNew = _status)"
          ></Task>
          <div v-else class="ms-n2">
            <button
            ...
            >
              <font-awesome-icon
                icon="fas fa-plus"
                class="text-primary fs-16px btn-icon"
              ></font-awesome-icon>
          ...
          <div class="ms-n2">
            <button
              @click="toggleworkflowModal = true"
              class="task-action-btn add-new"
            >
              <font-awesome-icon
                ...
                class="text-primary fs-16px btn-icon"
              ></font-awesome-icon>
              <span class="ms-2 fw-bold text-primary fs-14px"
                >Begin a workflow</span
              >
            </button>
          </div>
        </div>
  ...
  <workflow-modal
    v-if="toggleworkflowModal"
    @close="() => (toggleworkflowModal = false)"
  ></workflow-modal>
</template>
