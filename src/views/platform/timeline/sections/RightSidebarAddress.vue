...
<template>
  <div>
    ...
    <div class="mb-1">
      ...
      <div class="d-flex">
        <div v-if="!isFirstLoading" class="fs-14px fw-bold text-head mb-0">
          {{ address ?? " — " }}
        </div>

        <Skeletor style="width: 50%; height: 0.6rem" v-else></Skeletor>
      ...
      <div class="d-flex">
        <div v-if="!isFirstLoading" class="fs-14px fw-bold text-head mb-0">
          <span
            ...
            v-if="!isEditEstimatedValue"
            >${{ editLead?.estimated_value }}</span
          >
          <vue-number
            v-else
