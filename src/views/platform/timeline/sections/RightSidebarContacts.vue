...
const toggleDeleteContactModal = ref(false);
...
async function deleteLeadContactHandler(id) {
  await useApiRequest({
    ...
    .then((res) => {
      ...
      if (success) {
        toggleDeleteContactModal.value = false;
...
<template>
  ...
  >
    <div
      class="left ps-2 ms-2 py-1 d-flex justify-content-start align-items-center"
    >
      <div
        v-if="!isFirstLoading"
        ...
        :class="`${
          primaryContact.contact_id == item.contact_id ? 'shadow-border' : ''
        }`"
  ...
  <div class="px-3 py-2 border-bottom">
    ...
    >
      <h3
        class="user-name fs-18px fw-bold text-head mb-0"
        v-if="!isFirstLoading"
      >
        {{ primaryContact?.full_name }}
      </h3>
      <Skeletor v-else width="70%" style="height: 1rem" />
      <button
      ...
      >
        ...
        >
        <span
          v-if="!primaryContact?.is_primary"
          @click="toggleDeleteContactModal = true"
          class="dropdown-item py-1 cursor-pointer"
          >Remove Contact</span
        >
      </div>
    </div>
    <div>
      <table v-if="isFirstLoading" class="tbl-contact-info w-100">
        <tr v-for="(item, index) in [55, 70]" :key="index">
          <td
            class="d-flex justify-content-between align-items-center mb-2"
            width="100%"
          >
            <Skeletor :width="`${item}%`" style="height: 0.8rem" />
            <div
      ...
      <table v-if="!isFirstLoading" class="tbl-contact-info w-100">
        <tr v-if="primaryContact?.email">
          ...
          <td class="fs-16px">
            <a
              class="d-block user-email text-soft"
              :href="`mailto:${primaryContact?.email}`"
              >{{ primaryContact?.email }}</a
            >
          </td>
          <td class="hover" width="20%">
            <div class="d-flex justify-content-end align-items-center">
              <button
                @click="copyClipboardHandler(primaryContact?.email)"
        ...
        <tr
          v-for="(item, index) in primaryContact?.another_emails ?? []"
          :key="index"
          v-show="item.email"
        >
          <td class="fs-16px" width="10%">
          ...
          <td class="hover" width="20%">
            <div class="d-flex justify-content-end align-items-center">
              <button
                @click="copyClipboardHandler(item?.email)"
                class="btn btn-sm btn-light btn-floating bg-transparent d-flex justify-content-center align-items-center"
              >
                <font-awesome-icon
                  icon="fas fa-copy"
        ...
        <tr v-if="primaryContact?.phone_number">
          <td class="fs-16px" width="10%">
            <font-awesome-icon
              icon="fas fa-phone"
              class="text-soft fs-15px"
            ></font-awesome-icon>
          </td>
          <td class="fs-16px">
            <a
              class="d-block text-soft user-phone"
              :href="`tel:${primaryContact?.phone_number}`"
              >{{ primaryContact?.phone_number }}</a
            >
          ...
          <td class="hover" width="20%">
            <div class="d-flex justify-content-end align-items-center">
              <button
                @click="copyClipboardHandler(primaryContact?.phone_number)"
                class="btn btn-sm btn-light btn-floating bg-transparent d-flex justify-content-center align-items-center"
              >
                <font-awesome-icon
                  icon="fas fa-copy"
        ...
        >
          ...
          <td @click="handleContactEdit(true, primaryContact)" class="fs-16px text-soft">Add phone</td>
        </tr>
        <tr
          v-for="(item, index) in primaryContact?.another_phones ?? []"
          :key="index"
        ...
        >
          ...
          <td class="hover" width="20%">
            <div class="d-flex justify-content-end align-items-center">
              <button
                @click="copyClipboardHandler(item?.phone_number)"
                class="btn btn-sm btn-light btn-floating bg-transparent d-flex justify-content-center align-items-center"
              >
                <font-awesome-icon
                  icon="fas fa-copy"
  ...
  <!-- Modal Area -->
  <contact-edit-modal
    v-if="toggleContactEditModal"
    :contact-edit="contactEditModal"
    @close="() => (toggleContactEditModal = false)"
  ></contact-edit-modal>
  <!-- Delete Contact Confirm Modal -->
  <bootstrap-modal
    v-if="toggleDeleteContactModal"
    @close="() => (toggleDeleteContactModal = false)"
    size="sm"
  >
