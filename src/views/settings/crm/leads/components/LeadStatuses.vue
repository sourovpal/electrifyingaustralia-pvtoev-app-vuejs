<script>
    ...
    export default {
        ...
        data() {
            return {
                leadStatuses: [],
                exampleStatus: [{ id: 0, name: 'Example Status' }],
                isSubmitLeadStatus: false,
            }
        },
        methods: {
            async fetchLeadStaus() {
                try {
            ...
            <div class="list-group ">
                <vue-draggable-next class="lead-status-list"
                    ...
                    <div class="list-group-item py-0 border-bottom-0 border-top"
                        :class="item.is_lost == 1?'is-lost':null"
                        v-for="(item, index) in leadStatuses??exampleStatus"
                        :key="item.id">
                        <div class="box-info">
                            <div class="handle">
                                <svg width="24"
                            ...
                            <div class="action-dropdown">
                                ...
                                <ul class="dropdown-menu dropdown-menu-end">
                                    <li>
                                        <a class="dropdown-item lost-item"
                                            v-if="item.is_lost == 0"
                                            @click="item.is_lost = 1">Make a lost</a>
                                    </li>
                                    <li>
                                        <a class="dropdown-item active-item"
                                            v-if="item.is_lost == 1"
                                            @click="item.is_lost = 0">Make a active</a>
                                    </li>
                                    <li>
                                        <a class="dropdown-item delete-item"
                                            @click="leadStatuses.splice(index, 1)">Delete item</a>
                                    </li>
                                </ul>
                            </div>
                        </div>
                ...
                <div class="list-group-item bg-light text-center add-new-lead-status"
                    @click="leadStatuses.push({name:`New Status`, id:leadStatuses.length+1, is_lost:0})">Add New Status
                </div>
            </div>
            <div class="mt-3">
                <LoadingButton @submit="updateLeadStatusHandler"
                    :isLoading="isSubmitLeadStatus"
                    :disabled="isSubmitLeadStatus">
                    Save Settings
                </LoadingButton>
            </div>
        </div>
