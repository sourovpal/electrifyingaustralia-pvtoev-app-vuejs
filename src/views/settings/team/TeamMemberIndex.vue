<script>
    ...
    import { FetchUsersWithPagination } from '../../../actions/UserAction';
        ...
        methods: {
            async fetchMemberDataHandler(page = this.pagination.current_page, limit = 25) {
                try {
                    ...
                    this.isLoading = true;
                    const res = await FetchUsersWithPagination({ page, limit });
            ...
            selectedAllRowsHandler() {
                ...
                if (this.isSelectedAllRowsReset) {
                    ...
                    this.fetchTeamMembers.map((item) => {
                        this.selectedRows.push(item.user_id);
                    });
                } else {
                    this.selectedRows = [];
                    this.isSelectedAllRows = false;
...
<template>
    <section class="content team-members">
        ...
        <div class="content-body- border-top">
            <action-bar>
                ...
                <right-action-bar>
                    ...
                    <div class="me-3">
                        <button @click="showCreatememberModal"
            ...
            <Datatable>
                ...
                <datatable-header v-if="isFirstLoading || fetchTeamMembers.length">
                    ...
                    <div class="tbl-th"
                        style="width:20rem;">Team member</div>
                    <div class="tbl-th "
                        style="width:10rem;">Access Role</div>
                    <div class="tbl-th"
                ...
                <datatable-body>
                    <div v-if="!isFirstLoading || fetchTeamMembers.length"
                        :class="selectedRows.includes(member.user_id)?'active':''"
                        class="tbl-tr full-width"
                        v-for="(member, index) in fetchTeamMembers"
                        :key="index">

                        <div style="width:4rem;margin-left: -7px;"
                            class="tbl-td full-width">
                            <custom-checkbox @click="singleRowSelectedHandler(member.user_id)"
                                :checked="selectedRows.includes(member.user_id)" />
                        </div>

                        <div style="width:20rem;"
                            class="tbl-td">
                            <div @click="editTeamMemberHandler(member)"
                        ...
                        <div style="width:10rem;"
                            ...
                            <span v-else-if="member.user_role"
                                class="text-overflow-ellipsis btn btn-sm btn-success py-0 px-2">{{ member.user_role }}</span>
                            <span v-else
                        ...
                        <div style="width:10rem;flex-grow: 1;"
                            ...
                            <span v-if="!member.invited_at"
                                ...
                                member.invited_at??'Pending invitations' }}</span>
                            <span v-if="member.invited_at"
                                class="text-overflow-ellipsis btn btn-sm btn-success py-0 px-2">{{ member.invited_at
                                }}</span>
                        </div>
                        ...
                        <div style="width:10rem;"
                            class="tbl-td d-none d-lg-flex">{{ member.updated_ago }}</div>

                        <div style="width:10rem;"
                            class="tbl-td d-none d-lg-flex">{{ member.created_at }}</div>
                    </div>
        ...
        <invite-new-member-modal ref="inviteNewMemberModalRef"
            :fetchMemberDataHandler="fetchMemberDataHandler" />

        <edit-team-member-modal ref="editTeamMemberModalRef"
            :fetchMemberDataHandler="fetchMemberDataHandler" />
